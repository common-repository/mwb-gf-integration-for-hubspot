<?php
/**
 * The complete management for the Hubspot-GF feeds custom post type.
 *
 * @since      1.0.0
 ...
 * @author     MakeWebBetter <https://makewebbetter.com>
 */
class Mwb_Gf_Integration_Hubspot_Feed_Module {
	...
	public function __construct() {
		...
		$this->admin = 'Mwb_Gf_Integration_For_' . $this->crm_name . '_Crm_Admin';

	}

	/**
	 * Main Mwb_Gf_Integration_Hubspot_Feed_Module Instance.
	 *
	 * Ensures only one instance of Mwb_Gf_Integration_Hubspot_Feed_Module is loaded or can be loaded.
	 *
	 * @since 1.0.0
	 *
	 * @static
	 * @return Mwb_Gf_Integration_Hubspot_Feed_Module - Main instance.
	 */
	...
	public function register_feeds_post_type() {
		...
		$labels = array(
			'name'               => _x( 'Feeds', 'Post Type General Name', 'mwb-gf-integration-for-hubspot' ),
			'singular_name'      => _x( 'Feed', 'Post Type Singular Name', 'mwb-gf-integration-for-hubspot' ),
			'menu_name'          => _x( 'Crm Feeds', 'Admin menu name', 'mwb-gf-integration-for-hubspot' ),
			'parent_item_colon'  => __( 'Parent Feed', 'mwb-gf-integration-for-hubspot' ),
			'all_items'          => __( 'All Feeds', 'mwb-gf-integration-for-hubspot' ),
			'view_item'          => __( 'View Feed', 'mwb-gf-integration-for-hubspot' ),
			'add_new_item'       => __( 'Add New Feed', 'mwb-gf-integration-for-hubspot' ),
			'add_new'            => __( 'Add New', 'mwb-gf-integration-for-hubspot' ),
			'edit_item'          => __( 'Edit Feed', 'mwb-gf-integration-for-hubspot' ),
			'update_item'        => __( 'Update Feed', 'mwb-gf-integration-for-hubspot' ),
			'search_items'       => __( 'Search Feed', 'mwb-gf-integration-for-hubspot' ),
			'not_found'          => __( 'Not Found', 'mwb-gf-integration-for-hubspot' ),
			'not_found_in_trash' => __( 'Not found in Trash', 'mwb-gf-integration-for-hubspot' ),
		);

		$args = array(
			'label'                => __( 'Feeds', 'mwb-gf-integration-for-hubspot' ),
			'description'          => __( 'Feeds for crm', 'mwb-gf-integration-for-hubspot' ),
			'labels'               => $labels,
	...
	public function add_feed_meta_box() {
		...
		$metaboxes[] = array(
			...
			'title'    => esc_html__( 'Feed details', 'mwb-gf-integration-for-hubspot' ),
		...
		if ( ! $dependent ) {
			$metaboxes[] = array(
				...
				'title'    => esc_html__( 'Conditional Statements', 'mwb-gf-integration-for-hubspot' ),
		...
		foreach ( $metaboxes as $key => $metabox ) {
			...
			if ( class_exists( 'Gf_Integration_For_' . $this->crm_name . '_Feed_Extension' ) && ! method_exists( $this, $metabox['callback'] ) ) {
	...
	public function get_pro_callback( $key = '' ) {
		...
		$pro       = 'Gf_Integration_For_' . $this->crm_name . '_Feed_Extension';
		$pro_class = $pro::get_instance();
		$result    = '';
		switch ( $key ) {

			case 'pro-manual-sync':
				$result = array( $pro_class, 'mwb_gf_int_pro_feeds_pro_manual_sync' );
				break;

			case 'pro-add-notes':
				$result = array( $pro_class, 'mwb_gf_int_pro_feeds_pro_add_notes' );
				break;

			case 'pro-associate-user':
				$result = array( $pro_class, 'mwb_gf_int_pro_feeds_associate_user' );
				break;

			case 'pro-associate-category':
				$result = array( $pro_class, 'mwb_gf_int_pro_feeds_associate_category' );
				break;

			case 'pro-associate-project':
				$result = array( $pro_class, 'mwb_gf_int_pro_feeds_associate_project' );
				break;

			case 'pro-associate-organisation':
				$result = array( $pro_class, 'mwb_gf_int_pro_feeds_associate_org' );
				break;

			case 'pro-associate-opportunity':
				$result = array( $pro_class, 'mwb_gf_int_pro_feeds_associate_opp' );
				break;

			case 'pro-add-tags':
				$result = array( $pro_class, 'mwb_gf_int_pro_feeds_add_tags' );
				break;

			case 'pro-add-lead':
				$result = array( $pro_class, 'mwb_gf_int_pro_feeds_pro_add_lead' );
				break;

			case 'pro-add-attahcment':
				$result = array( $pro_class, 'mwb_gf_int_pro_feeds_pro_add_attachments' );
				break;
	...
	public function feeds_mb_condition_render() {
		...
		$params['enable_filter'] = $this->fetch_feed_data( $post->ID, 'mwb-' . $this->crm_slug . '-gf-enable-filters', '' );
	...
	private function render_mb_data( $meta_box = false, $params = array() ) {
		...
		$params['admin_class'] = 'Mwb_Gf_Integration_For_' . $this->crm_name . '_Crm_Admin';
	...
	public function save_feeds_data( $post_id ) {
		...
		if ( isset( $_POST['post_type'] ) && $this->feed_name == $_POST['post_type'] ) { // phpcs:ignore
			...
			$crm_fields       = isset( $_POST['mwb-hs-object-fields'] ) ? map_deep( wp_unslash( $_POST['mwb-hs-object-fields'] ), 'sanitize_text_field' ) : array();
			...
			update_post_meta( $post_id, 'mwb-' . $this->crm_slug . '-gf-enable-filters', $enable_filters );
			update_post_meta( $post_id, 'mwb-' . $this->crm_slug . '-gf-condtion-field', $condition );
			update_post_meta( $post_id, 'mwb-' . $this->crm_slug . '-gf-crm-fields', $crm_fields );
