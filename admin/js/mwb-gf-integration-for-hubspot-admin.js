...
jQuery( document ).ready( function() {
	...
	const crm          = mwb_gf_integration_ajax_data.crm
	const ajaxUrl      = mwb_gf_integration_ajax_data.ajaxUrl;
	const nonce        = mwb_gf_integration_ajax_data.ajaxNonce;
	const action       = mwb_gf_integration_ajax_data.ajaxAction;
	const error        = mwb_gf_integration_ajax_data.criticalError;
	const feedBackLink = mwb_gf_integration_ajax_data.feedBackLink;
	const feedBackText = mwb_gf_integration_ajax_data.feedBackText;
	const isPage       = mwb_gf_integration_ajax_data.isPage;
	const trashIcon    = mwb_gf_integration_ajax_data.trashIcon;
	const apiKeyImg    = mwb_gf_integration_ajax_data.api_key_image;
	 ...
	 * @return  void
	 */
	 const triggerSetupDocs = ( crm, content, title ) => {
		...
		result.then( ( response ) => {
			if ( true == response.status ) {
				...
				if ( response.data.hasOwnProperty( 'html' ) ) {
					jQuery( '.mwb_gf_integration__feeds-wrap' ).nextAll( '.mwb-content-wrap' ).slideUp();
					jQuery( '.mwb_gf_integration__feeds-wrap' ).nextAll( '.mwb-content-wrap' ).remove();
					jQuery( '.mwb-feed__select-object' ).closest( 'div.mwb_gf_integration__feeds-wrap' ).parent().append( response.data.html );
	 ...
	 * @return   html.
	 */
	 const createAddNoteField = ( formFields ) => {

		if ( jQuery( 'body' ).hasClass( 'post-type-mwb_' + crm + '_gf' ) ) {
			
			let html = '<option value="-1">Select option</option>';
			...
			jQuery( '#mwb-' + crm + '-add-note-select' ).html( html );
		}
	 ...
	 * @return   html.
	 */
	 const createAddTagField = ( formFields ) => {
		...
		if ( jQuery( 'body' ).hasClass( 'post-type-mwb_' + crm + '_gf' ) ) {
			...
			jQuery( '#mwb-' + crm + '-add-tag-select' ).html( html );
		}
	...
	const loadIntegrationFields = () => {
		...
		switch( method ) {
			case 'api':
				jQuery( '.mwb_sf_gf_table' ).find( '.mwb-api-fields' ).slideDown();
				jQuery( '.mwb_sf_gf_table' ).find( '.mwb-web-fields' ).slideUp();
				break;

			case 'web':
				jQuery( '.mwb_sf_gf_table' ).find( '.mwb-api-fields' ).slideUp();
				jQuery( '.mwb_sf_gf_table' ).find( '.mwb-web-fields' ).slideDown();
				break;
		}
	}
	...
	const manageAssociationFields = ( objects = false ) => {
		...
		if ( jQuery( 'body' ).hasClass( 'post-type-mwb_' + crm + '_gf' ) ) {
			...
			jQuery( '#mwb_' + crm + '_gf_feeds_associate_user_meta_box' ).css( 'display', 'none' );
			jQuery( '#mwb_' + crm + '_gf_feeds_associate_category_meta_box' ).css( 'display', 'none' );
			jQuery( '#mwb_' + crm + '_gf_feeds_associate_project_meta_box' ).css( 'display', 'none' );
			jQuery( '#mwb_' + crm + '_gf_feeds_associate_org_meta_box' ).css( 'display', 'none' );
			jQuery( '#mwb_' + crm + '_gf_feeds_associate_opp_meta_box' ).css( 'display', 'none' );
			jQuery( '#mwb_' + crm + '_gf_feeds_add_lead_meta_box' ).css( 'display', 'none' );
			jQuery( '#mwb_' + crm + '_gf_feeds_add_tag_meta_box' ).css( 'display', 'none' );
			...
			if ( dependent.length > 0 ) {
				...
				jQuery( '#mwb-' + crm + '-gf-select-form option:not(:selected)' ).attr( 'disabled', 'disabled' );
				jQuery( '#mwb-feeds-' + crm + '-object option:not(:selected)' ).attr( 'disabled', 'disabled' );
			} else {
				...
				switch ( objects ) {
					case 'Contacts':
						jQuery( '#mwb_' + crm + '_gf_feeds_associate_org_meta_box' ).slideDown();
						jQuery( '#mwb_' + crm + '_gf_feeds_add_tag_meta_box' ).slideDown();
						break;
					...
					case 'Opportunities':
						jQuery( '#mwb_' + crm + '_gf_feeds_associate_user_meta_box' ).slideDown();
						jQuery( '#mwb_' + crm + '_gf_feeds_associate_category_meta_box' ).slideDown();
						jQuery( '#mwb_' + crm + '_gf_feeds_associate_org_meta_box' ).slideDown();
						jQuery( '#mwb_' + crm + '_gf_feeds_add_tag_meta_box' ).slideDown();
						break;
					...
					case 'Leads':
						...
						jQuery( '#mwb_' + crm + '_gf_feeds_associate_user_meta_box' ).slideDown();
						jQuery( '#mwb_' + crm + '_gf_feeds_associate_category_meta_box' ).slideUp();
					...
					case 'Tasks':
						...
						jQuery( '#mwb_' + crm + '_gf_feeds_associate_user_meta_box' ).slideDown();
						jQuery( '#mwb_' + crm + '_gf_feeds_associate_category_meta_box' ).slideDown();
						jQuery( '#mwb_' + crm + '_gf_feeds_associate_opp_meta_box' ).slideDown();
						jQuery( '#mwb_' + crm + '_gf_feeds_associate_project_meta_box' ).slideDown();
						jQuery( '#mwb_' + crm + '_gf_feeds_add_tag_meta_box' ).slideUp();
						break;
					...
					case 'Prospects':
						jQuery( '#mwb_' + crm + '_gf_feeds_add_lead_meta_box' ).slideUp();
						jQuery( '#mwb_' + crm + '_gf_feeds_associate_user_meta_box' ).slideUp();
						jQuery( '#mwb_' + crm + '_gf_feeds_associate_category_meta_box' ).slideUp();
						jQuery( '#mwb_' + crm + '_gf_feeds_associate_contact_meta_box' ).slideDown();
						jQuery( '#mwb_' + crm + '_gf_feeds_add_tag_meta_box' ).slideUp();
						break;
	...
	/**
	 * Manage auth form fields. 
	 * 
	 * @since   1.0.3
	 * @returns void
	 */
	 const manageAuthForm = ( method = false ) => {

		let form   = jQuery( '#mwb_gf_integration_account_form' );
		
		if ( false == method ) {
			method = jQuery( '#mwb-' + crm + '-gf-own-app' ).is( ':checked' );
		}

		if ( form.length > 0 ) {
			let api_fields = form.find( 'tr.mwb-api-fields' );
			let setup      = jQuery( '.mwb-intro__bottom-text-wrap' ).find( 'p.mwb-api-fields' );
	...
	jQuery( '#mwb_gf_integration_settings_form' ).on( 'submit', function( e ) {
		e.preventDefault();
	...
	jQuery( '#mwb-showauth-form' ).on( 'click', function( e ) {
		...
		jQuery( '.mwb-intro' ).slideUp( '400', function() {
			jQuery( '.mwb_gf_integration_account-wrap' ).slideDown();
		} );
	} );
	...
	jQuery( document ).on( 'click', '.trigger-setup-guide', function() {
        ...
        let custom_app = jQuery( '#mwb-hubspot-gf-own-app' ).is( ':checked' ) ? 'yes' : 'no';
		let input      = {
			action : action,
			nonce  : nonce,
			event  : event,
		...
		result.then( ( response ) => {
			if ( true == response.status ) {
				if ( true == response.data.success ) {
					triggerSetupDocs( crm, response.data.content, response.data.title );
				}
			} else {
				triggerError();
			}
	...
	jQuery( '.mwb-section__sub-heading__wrap' ).on( 'click', function() {
		...
		jQuery( this ).siblings( '.mwb-gf-integration__status-wrap' ).slideToggle( 300 );
	} );
	...
	jQuery( 'input[name="mwb_account[own_app]"]' ).on( 'change', function() {
		manageAuthForm();
	} );
	...
	jQuery( '#mwb-' + crm + '-gf-authorize-button' ).on( 'click', function( e ) {
		...
		let client_id = jQuery( '#mwb-hubspot-gf-client-id' ).val();
		let secret_id = jQuery( '#mwb-hubspot-gf-client-secret' ).val();
		let scopes    = jQuery( '#mwb-hubspot-gf-scopes' ).val();
		let method    = jQuery( '#mwb-hubspot-gf-own-app' ).is( ':checked' );
		
		if ( method ) {
			if ( client_id.length == 0 || secret_id.length == 0 || scopes.length == 0 ) {
				...
				href = updateQueryStringParameter( href, 'scopes', scopes );
			}
		} 
		
		href = updateQueryStringParameter( href, 'method', method );
	...
	jQuery( '#mwb_gf_integration_refresh_token' ).on( 'click', function( e ) {
		...
		result.then( ( response ) => {
			...
			if ( true == response.status ) {
				if ( true == response.data.success ) {
					...
					jQuery( '#mwb-gf-token-expiry-notice' ).text( response.data.token_message );
				} else {
					triggerError( response.data.msg );
				}
			} else {
	...
	jQuery( '#mwb_gf_integration_reauthorize' ).on( 'click', function( e ) {
		e.preventDefault();
		Swal.fire({
			title: 'Are you sure?',
			...
			confirmButtonText: 'Yes, Reauthorize!'
		  }).then((result) => {
	...
	/* Disconnect account */
	jQuery( '#mwb_gf_integration_disconnect' ).on( 'click', function( e ) {
		e.preventDefault();
	...
	/* Disabling other forms and objects so that user can't change once feed is published */
	var val = jQuery( '#mwb-' + crm + '-gf-select-form' ).closest( '#poststuff' ).find( '#publish' ).val();

	if ( 'Update' == val || jQuery( '#mwb_' + crm + '_gf_feeds_condition_meta_box' ).length < 1 ) {
		jQuery( '#mwb-' + crm + '-gf-select-form option:not(:selected)' ).attr( 'disabled', 'disabled' );
		jQuery( '#mwb-feeds-' + crm + '-object option:not(:selected)' ).attr( 'disabled', 'disabled' );
	}
	...
	jQuery( '#mwb-' + crm + '-gf-select-form' ).on( 'change', function() {
		...
		result.then( ( response ) => {
			if ( true == response.status ) {
				...
				createAddNoteField( formFields );
				createAddTagField( formFields );
	...
	jQuery( '#mwb-feeds-' + crm + '-object' ).on( 'change', function() {
		...
		// Remove previous content html first.
		jQuery('.mwb_gf_integration__feeds-wrap').nextAll('.mwb-content-wrap').slideUp();
		jQuery('.mwb_gf_integration__feeds-wrap').nextAll('.mwb-content-wrap').remove();
	...
	jQuery( '#mwb-' + crm + '-refresh-fields' ).on( 'click', function() {
		...
		if ( '-1' == module ) {
			triggerError( 'Please select a HubSpot Object first' );
			return;
		}
	...
	jQuery( document ).on( 'click' , '#add-new-field-btn', function( e ) {
		...
		if( '-1' == object || '-1' == field ) {
			triggerInfo( 'Select a valid HubSpot object and its corresponding field' );
			return;
		}
	...
	/* Tag field select */
	jQuery( document ).on( 'change', '#mwb-' + crm + '-add-tag-select', function( e ) {
		let type_select = jQuery( this );
		let field_value = type_select.val();
		let input       = jQuery( '#add-tags' ).val();

		jQuery( '#add-tags' ).val( input + '{' + field_value + '}' );
	} );
	...
	jQuery( '.mwb_gf_integration_trash_feed' ).on( 'click', function() {
	...
	jQuery( document ).on( 'change', '.filter-feeds-by-form', function() {
		...
		result.then( ( response ) => {
			if ( true == response.status ) {
				if ( true == response.data.status ) {
					jQuery( '.mwb-gf-integration__feed-list' ).slideUp();
					jQuery( '.mwb-gf-integration__feed-list' ).html( response.data.feeds );
					jQuery( '.mwb-gf-integration__feed-list' ).slideDown();
				}
			} else {
	...
	/* Toggle feeds fields */
	jQuery( '.mwb_gf_integration__feeds-wrap .mwb-feeds__meta-box-main-wrapper' ).slideDown();
	jQuery( '.mwb_gf_integration__feeds-wrap .mwb-feeds__header-link' ).addClass( 'link-active' );
